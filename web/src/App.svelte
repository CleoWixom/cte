<script>
  ...
  onMount(async () => {
    try {
      // Path is resolved at runtime; wasm-pack places files here during CI.
      const wasmUrl = new URL('/wasm/trieval_wasm.js', window.location.href).href
      const mod = await import(/* @vite-ignore */ wasmUrl)
      await mod.default()
      engine = new mod.TriangulationEngine()
      wasmReady = true
    } catch (e) {
...
<main>
  ...
  <div class="layout">
    <div class="map-container">
      <MapView {selectedPoint} {result} {loading} on:pointSelected={onPointSelected} />
    </div>
    <aside class="sidebar">
...
<style>
  ...
  header {
    ...
    border-bottom: 1px solid #2d3148; display: flex; align-items: baseline; gap: 16px;
  }
  h1 { margin: 0; font-size: 1.25rem; font-weight: 700; color: #7c83ff; }
  .subtitle { color: #64748b; font-size: 0.85rem; }
  .layout { display: flex; flex: 1; overflow: hidden; }
  .map-container { flex: 1; position: relative; }
  .sidebar {
    width: 320px; min-width: 280px; overflow-y: auto; background: #1a1d27;
    border-left: 1px solid #2d3148; padding: 16px; display: flex; flex-direction: column; gap: 16px;
  }
