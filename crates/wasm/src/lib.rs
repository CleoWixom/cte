...
use trieval_core::{
    ...
    metrics::{compute_metrics, AccuracyMetrics},
    montecarlo::{monte_carlo, McPoint, MonteCarloInput},
    signal::{normalize_to_dbm, RadioType},
...
impl TriangulationEngine {
    ...
    pub fn new() -> Self {
        // Better panic messages in WASM console
    ...
    /// Load tower data from JS.
    /// Expected: `[{id, radio, lat, lon, range_m?}]`
    ...
    /// Load measurement data from JS.
    /// Expected: `[{cell_id, lat, lon, signal_dbm, reliability?}]`
    ...
    /// Set interpolation model: "kriging" | "idw"
    pub fn set_model(&mut self, model: &str) {
        self.use_kriging = model == "kriging";
    }

    /// Set number of Monte Carlo iterations (default 300).
    pub fn set_mc_iterations(&mut self, n: usize) {
        self.mc_iterations = n.clamp(50, 1000);
    }

    /// Run triangulation for the given query point (used as MC truth reference).
    /// Returns `TriangulationResult` as JsValue.
    pub fn solve(&self, query_lat: f64, query_lon: f64) -> JsValue {
        match self.run(query_lat, query_lon) {
            ...
            Err(e) => {
                web_sys::console::error_1(&JsValue::from_str(&format!("trieval error: {e}")));
                JsValue::NULL
            }
        }
    }
    ...
    fn run(&self, query_lat: f64, query_lon: f64) -> Result<TriangulationResult, String> {
        ...
        // Build per-tower spatial models from measurements
        ...
        for tower in &self.towers {
            ...
            if ms.is_empty() {
                continue; // Skip towers with no measurements
            }

            total_measurements += ms.len();

            // Observed RSSI at query point = model prediction
        ...
        if n_towers < 2 {
            return Err(format!(
                "need ≥2 towers with measurements, got {n_towers}"
            ));
        }
        ...
        // Metrics (use LM result as truth reference for CEP)
        ...
        let cloud_points: Vec<CloudPoint> = cloud
            .iter()
            .map(|p| CloudPoint { lat: p.lat, lon: p.lon })
            .collect();

        let model_type = if spatial_models.iter().any(|m| matches!(m, SpatialModel::Kriging(_))) {
            "kriging".into()
        } else {
            "idw".into()
        };
